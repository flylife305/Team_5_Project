...
def open_calculator(username):
    ...
    tk.Label(root, text="Down Payment($):", bg="lightblue").grid(row=1, column=0, padx=10, pady=5, sticky="e")
    ...
    tk.Label(root, text="Loan Term (months):", bg="lightblue").grid(row=2, column=0, padx=10, pady=5, sticky="e")
    term_entry = tk.Entry(root)
    term_entry.grid(row=2, column=1, padx=10, pady=5)
    term_entry.insert(0, user_data.get("loan_term", ""))

    tk.Label(root, text="Credit Score", bg="lightblue").grid(row=3, column=0, padx=10, pady=5, sticky="e")
...
#####Calculation Function#####
    ...
    def calculate_payment():
        try:
            ...
            if loan_term < 1 or loan_term > 360:
                messagebox.showerror("Invalid Input", "Loan term must be between 1 and 360 months.")
                return

            credit_score = int(credit_score_entry.get())
            if credit_score < 300 or credit_score > 850:
                messagebox.showerror("Invalid Input", "Credit score must be between 300 and 850.")
                return
            ...
            monthly_payment_label.config(text=f"Monthly Payment: ${monthly_payment:,.2f}", fg="green")
    ...
    def save_data():
        try:
            ...
            messagebox.showerror("Error", "Loan term must be a number between 1 and 360.")
            return
        ...
        try:
            ...
            messagebox.showerror("Error", "Credit score must be a number between 300 and 850.")
            return
    ...
    for col in columns:
        ...
        tree.column(col, anchor="center", width=135)
            ...
            for username, user_info in users_data.items():
                ...
                if mortgage:
                    ...
                    INSERT OR REPLACE INTO mortgage_data (
                        username, price, interest_rate, down_payment, loan_term, credit_score, credit_class
                    ) VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (
                        username,
                        mortgage.get("price", ""),
